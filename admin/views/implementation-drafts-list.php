...
<div class="wrap rwgc-wrap rwga-wrap rwga-wrap--implementation-drafts">
	<?php if ( class_exists( 'RWGC_Admin_UI', false ) ) : ?>
		...
		RWGC_Admin_UI::render_page_header(
			...
			__( 'Reviewable copy variants from the bounded copy workflow (not published automatically).', 'reactwoo-geo-ai' )
		);
		?>
	<?php else : ?>
	...
	<div class="rwgc-card">
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<?php submit_button( __( 'Generate drafts', 'reactwoo-geo-ai' ), 'primary', 'submit', false ); ?>
		</form>
	</div>
	...
	<div class="rwgc-card">
		<form method="get" action="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>" class="rwga-rec-filter">
			...
			<label for="rwga-filter-rec"><?php esc_html_e( 'Filter by recommendation ID', 'reactwoo-geo-ai' ); ?></label>
		...
		<?php else : ?>
			...
			if ( $total_pages > 1 ) {
				...
				if ( $rwga_filter_recommendation > 0 ) {
					$base_url = add_query_arg( 'recommendation_id', $rwga_filter_recommendation, $list_url );
				}
